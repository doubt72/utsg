...
function hexIntersection(
  hex: Hex, p0: Coordinate, p1: Coordinate, fromEdge: Direction, fromCorner: Direction
): IntersectionData {
  for (let i = 0; i < 6; i++) {
    ...
    if (lineOrientation(p0, h0, p1) === orientation.Straight &&
        h0.onSegment(p0, p1) && i+1 !== fromCorner) {
...
export function losHexPath(map: Map, start: Hex, target: Hex): hexPathData[] {
  ...
  if (start.coord.x === target.coord.x && start.coord.y === target.coord.y) { return hexes }
  ...
  let count = 0
  let fromEdge: Direction = 1
  let fromCorner: Direction = 1
  while (hex.coord.x !== target.coord.x || hex.coord.y !== target.coord.y) {
    ...
    if (check.edge) { // Edge crossing
      ...
      fromCorner = 1
    } else if (check.corner) { // Corner crossings - travelling along hex edge
      ...
      fromEdge = 1
    } else if (check.cornerIntersection) { // Corner crossing - hex-to-hex
      // Technically we cross the edge at the...  Edge of the edge, then move
      // to next hex.  An...  Edge case, but can happen at long range
      const dir = normalDir(check.orientation === orientation.Clockwise ?
                          check.cornerIntersection : check.cornerIntersection - 1)
      hexes.push({ edge: dir, edgeHex: hex, long: false })
      hex = map.neighborAt(hex.coord, dir) as Hex
      hexes.push({ hex: hex })

      fromCorner = normalDir(check.orientation === orientation.Clockwise ?
                             check.cornerIntersection - 2 : check.cornerIntersection + 2)
      fromEdge = 1
    } else {
...
function elevationHindrance(
  start: Hex, target: Hex, elevation: number, currDist: number, hindrance: number
): number {
...
function hexElevationHindrance(start: Hex, target: Hex, hex: Hex): number {
  ...
  const currDist = start.elevation > target.elevation ? hexDistance(start, hex) :
...
function edgeElevationHindrance(start: Hex, target: Hex, hex: Hex, edge: Direction): number {
  const currDist = start.elevation > target.elevation ? hexDistance(start, hex) + 1 :
    hexDistance(hex, target)
  return elevationHindrance(start, target, hex.elevation, currDist, hex.edgeHindrance(edge))
}

function alongEdgeElevationHindrance(
  start: Hex, target: Hex, hex: Hex, edge: Direction, initialEdge: boolean
): number {
...
function elevationLos(start: Hex, target: Hex, elevation: number, currDist: number, los: boolean): boolean {
...
function alongEdgeElevationLos(
  start: Hex, target: Hex, hex: Hex, edge: Direction, initialEdge: boolean, finalEdge: boolean
): boolean {
  const neighbor = hex.map.neighborAt(hex.coord, edge)
...
export function los(map: Map, start: Coordinate, end: Coordinate): TextLayout | boolean {
  ...
  for (let i = 0; i < path.length; i++) {
    ...
    if (curr.edge) {
      if (curr.long) {
        hindrance += alongEdgeElevationHindrance(hex0, hex1, curr.edgeHex as Hex, curr.edge, i === 0)
        const los = alongEdgeElevationLos(
          hex0, hex1, curr.edgeHex as Hex, curr.edge, i === 0, i === path.length - 2
        )
        if (los) { return false }
      } else {
        if (i !== 0) {
        ...
        if (los && (hex?.coord.x !== end.x || hex.coord.y !== end.y) && (i !== 0)) { return false }
      }
    } else {
      hindrance += hexElevationHindrance(hex0, hex1, curr.hex as Hex)
      const block = hexElevationLos(hex0, hex1, curr.hex as Hex)
      if (block && (curr.hex?.coord.x !== end.x || curr.hex?.coord.y !== end.y)) { return false }
    }
  }
  ...
  return {
    value: hindrance, size: 80,
    x: lastHex.xOffset + offset, y: lastHex.yOffset + 24 - offset,
