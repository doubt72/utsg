...
import { GameActionBreakdowndData, GameActionData, GameActionDiceResult, GameActionUnit } from "../GameAction";
...
export default class BreakdownAction extends BaseAction {
  ...
  constructor(data: GameActionData, game: Game, index: number) {
    ...
    this.validate(data.data.breakdown_data)

    this.origin = (data.data.origin as GameActionUnit[])[0]
    this.diceResult = (data.data.dice_result as GameActionDiceResult[])[0]
    this.breakdownRoll = (data.data.breakdown_data as GameActionBreakdowndData).breakdown_roll
  }
