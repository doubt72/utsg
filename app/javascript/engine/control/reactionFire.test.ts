import { Coordinate, featureType, unitType } from "../../utilities/commonTypes"
...
describe("reaction fire attacks", () => {
  test("reaction fire after fire", () => {
    ...
    expect(game.lastAction?.stringValue).toBe(
      ...
      "target 15, rolled 1 [d10x10: 1 x 1]: miss, firing weapon broken")
  ...
  test("reaction fire after move", () => {
    ...
    expect(game.lastAction?.type).toBe("reaction_fire")
    expect(game.lastAction?.stringValue).toBe(
      "reaction fire: Soviet T-34 M40 at A3 fired at German Rifle at B3; targeting roll: target 3, " +
  ...
  test("team ghosts are correct", () => {
    ...
    expect(counters1.length).toBe(1)
    expect(counters1[0].unit.ghost).toBe(true)
    expect(counters1[0].unit.type).toBe(unitType.Team)
    const counters2 = map.countersAt(new Coordinate(2, 2))
    expect(counters2.length).toBe(1)
    expect(counters2[0].unit.ghost).toBe(true)
    expect(counters2[0].unit.type).toBe(unitType.Team)
  ...
  test("hit, morale check shorts move for unit", () => {
    ...
    expect(counters2.length).toBe(2)
    expect(counters2[0].marker.type).toBe("spotting")
    expect(counters2[1].unit.ghost).toBe(true)
    expect(counters2[1].unit.selected).toBe(true)
  ...
  test("ghost directions are correct, and vehicle is removed", () => {
    ...
    expect(game.lastAction?.stringValue).toBe(
      "reaction fire: Soviet T-34 M40 at A3 fired at German PzKpfw 35(t) at C3; targeting roll: " +
        "target 6, rolled 100 [d10x10: 10 x 10]: hit; hit location roll: 10 [d10] (hull); " +
        "penetration roll (front): target 8, " +
  ...
  test("hit shorts move for unit with proper orientation", () => {
    ...
    expect(game.lastAction?.stringValue).toBe(
      ...
      "targeting roll: target 6, rolled 16 [d10x10: 4 x 4]: hit; hit location roll: 4 [d10] (hull); " +
