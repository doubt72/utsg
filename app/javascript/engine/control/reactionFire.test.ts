...
describe("reaction fire attacks", () => {
  test("reaction fire after fire", () => {
    ...
    expect(game.lastAction?.stringValue).toBe(
      "reaction fire: Soviet T-34 M40 at A3 fired at German PzKpfw 35(t) at E3; targeting roll: " +
      "target 10, rolled 1 [d10x10: 1 x 1]: miss, firing weapon broken")
  ...
  test("reaction fire after move", () => {
    ...
    expect(game.lastAction?.type).toBe("reaction_fire")
    expect(game.lastAction?.stringValue).toBe(
      "reaction fire: Soviet T-34 M40 at A3 fired at German Rifle at B3; targeting roll: target 2, " +
      "rolled 1 [d10x10: 1 x 1]: miss, firing weapon broken")
    expect(game.moraleChecksNeeded).toStrictEqual([])
  ...
  test("reaction fire mid move", () => {
    ...
    expect(game.lastAction?.type).toBe("reaction_fire")
    expect(game.lastAction?.stringValue).toBe(
      "reaction fire: Soviet T-34 M40 at A3 fired at German Rifle at C3; targeting roll: target 4, " +
  ...
  test("multiple units moving", () => {
    ...
    expect(game.lastAction?.stringValue).toBe(
      ...
      "target 4, rolled 100 [d10x10: 10 x 10]: hit; roll for effect: target 10, rolled 20 [2d10: 10 + 10]: passed")
    expect(game.moraleChecksNeeded).toStrictEqual([
      { unit: unit, from: [new Coordinate(0, 2)], to: new Coordinate(2, 2), incendiary: false },
  ...
  test("multiple units moving, plus extra targeting", () => {
    ...
    expect(game.lastAction?.stringValue).toBe(
      ...
      "roll: target 4, rolled 100 [d10x10: 10 x 10]: hit; roll for effect: target 10, rolled 20 [2d10: 10 + 10]: passed")
    expect(game.moraleChecksNeeded).toStrictEqual([
  ...
  test("ghosts for dropping are correct", () => {
    ...
    expect(counters[1].unit.name).toBe("MG 08/15")
    counters = map.countersAt(new Coordinate(2, 2))
    expect(counters.length).toBe(2)
    expect(counters[0].unit.ghost).toBe(undefined)
    expect(counters[0].unit.name).toBe("MG 08/15")
    expect(counters[1].unit.ghost).toBe(true)
    expect(counters[1].unit.name).toBe("Rifle")
    counters = map.countersAt(new Coordinate(1, 2))
    ...
    expect(reactionFireHexes(game)).toStrictEqual([
      ...
      { x: 2, y: 2, facing: undefined, turret: undefined },
      { x: 1, y: 2, facing: undefined, turret: undefined },
      { x: 0, y: 2, facing: undefined, turret: undefined },
    ])
  })
  ...
  test("hit, morale check shorts move for unit", () => {
    ...
    const eloc = new Coordinate(1, 2)
    map.victoryHexes.push({ x: eloc.x, y: eloc.y, player: 1 })
    ...
    expect(map.victoryAt(eloc)).toBe(2)
  ...
  test("hit, morale check shorts move for unit, but not dropped unit", () => {
    const game = createFireGame()
    const map = game.scenario.map
  ...
  test("hit, morale check, rolls back dropped mg", () => {
    const game = createFireGame()
    const map = game.scenario.map
    const unit = new Unit(testGInf)
    unit.id = "test1"
    ...
    expect(counter?.hex?.y).toBe(1)
    expect(counter?.unit.children.length).toBe(0)
    counter = game.findCounterById(unit2.id)
    expect(counter?.hex?.x).toBe(1)
    expect(counter?.hex?.y).toBe(2)
    expect(counter?.unit.parent).toBe(undefined)
    ...
    expect(counter?.hex?.x).toBe(2)
    expect(counter?.hex?.y).toBe(2)
    expect(counter?.unit.children[0].id).toBe("test2")
    counter = game.findCounterById(unit2.id)
    expect(counter?.hex?.x).toBe(2)
    expect(counter?.hex?.y).toBe(2)
    expect(counter?.unit.parent?.id).toBe("test1")
  })

  test("hit, morale check, rolls back loaded mg", () => {
    const game = createFireGame()
    const map = game.scenario.map
    const unit = new Unit(testGInf)
    unit.id = "test1"
  ...
  test("ghost directions are correct, and vehicle is removed", () => {
    ...
    expect(game.lastAction?.type).toBe("reaction_fire")
    expect(game.lastAction?.stringValue).toBe(
      "reaction fire: Soviet T-34 M40 at A3 fired at German PzKpfw 35(t) at C3; targeting roll: " +
      "target 4, rolled 100 [d10x10: 10 x 10]: hit; hit location roll: 10 [d10] (hull); penetration roll (front): target 8, " +
  ...
  test("hit shorts move for unit with proper orientation", () => {
    ...
    expect(ghost.targetSelected).toBe(true)
    expect(fireHelpText(game, fireHex, counters[1].unit, true)).toStrictEqual([
      "attack rolls:",
      "-> targeting roll [d10x10]: 4 (92%)",
      "range: 2",
      "multiplier: 2",
      "- base multiplier 3",
      "- minus 1 for reaction fire",
      "",
    ...
    expect(game.lastAction?.stringValue).toBe(
      ...
      "targeting roll: target 4, rolled 16 [d10x10: 4 x 4]: hit; hit location roll: 4 [d10] (hull); " +
      "penetration roll (front): target 8, rolled 8 [2d10: 4 + 4]: tie, vehicle immobilized, move short at C3"
    )
