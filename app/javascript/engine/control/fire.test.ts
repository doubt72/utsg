...
describe("ranged fire attacks", () => {
  ...
  describe("fire action", () => {
    ...
    test("removing last unit in multiplayer stack flips VP", () => {
      const game = createBlankGame()
      const map = game.scenario.map
      const firing = new Unit(testRInf)
      firing.id = "firing1"
      ...
      map.toggleVP(tloc)
      expect(map.victoryAt(tloc)).toBe(2)
      game.setGameState(new FireState(game, false))
      ...
      expect(deHTML(game.lastAction?.stringValue as string)).toBe(
        "German morale check for Rifle (2d10): target 14, rolled 2 [2d10: 1 + 1], unit eliminated"
      )
      expect(map.victoryAt(tloc)).toBe(1)
    })
    ...
    test("offboard artillery doesn't target wrecks", () => {
      ...
      map.addCounter(tloc, target3)
      target3.wreck(game)
      const wreck = new Unit(testRTank)
      wreck.id = "wreck"
      map.addCounter(new Coordinate(4, 1), wreck)
      wreck.wreck(game)
      organizeStacks(map)
    ...
    test("gun firing smoke", () => {
      ...
      expect(all[3].unit.id).toBe("0-smoke-0-2")
      expect(all[3].hex?.x).toBe(0)
    ...
    test("immobilized vehicle unloads units", () => {
      ...
      expect(target2.isActivated).toBe(true)
      expect(target3.children.length).toBe(1)
      expect(target3.parent).toBe(undefined)
      expect(target3.isActivated).toBe(true)
      expect(target3.children.length).toBe(1)
      expect(target4.isActivated).toBe(true)
      expect(target5.parent).toBe(undefined)
      expect(target5.isActivated).toBe(true)
      expect(target6.isActivated).toBe(true)
    })
    ...
    test("destroyed vehicle unloads/breaks units", () => {
      ...
      expect(unit[0].marker.type).toBe("spotting")
      expect(unit[1].unit.name).toBe("M9 Half-track")
      expect(unit[1].unit.isWreck).toBe(true)
      expect(unit[1].unit.children.length).toBe(0)
      expect(unit[2].unit.name).toBe("45mm M-42")
      expect(unit[2].unit.isActivated).toBe(true)
      expect(unit[2].unit.parent).toBe(undefined)
      expect(unit[2].unit.facing).toBe(4)
      expect(unit[3].unit.name).toBe("Rifle")
      expect(unit[3].unit.isExhausted).toBe(true)
      expect(unit[3].unit.children.length).toBe(1)
      expect(unit[3].unit.parent).toBe(undefined)
      expect(unit[3].unit.isBroken).toBe(false)
      expect(unit[3].unit.children.length).toBe(1)
      expect(unit[5].unit.name).toBe("Leader")
      expect(unit[5].unit.isExhausted).toBe(true)
      expect(unit[5].unit.parent).toBe(undefined)
      expect(unit[5].unit.isBroken).toBe(false)
    ...
    test("ranged fire against armored vehicle", () => {
      ...
      expect(game.fireStartCheckNeeded).toStrictEqual([{
        ...
        vehicle_incendiary: false, nation: "ussr", player_nation: "ussr", tank: true,
      }])
    ...
    test("ranged fire hit no penetration", () => {
      const game = createFireGame()
      const map = game.scenario.map
      const firing = new Unit(testGTank)
      firing.id = "firing1"
      ...
      expect(target.isWreck).toBe(false)
      expect(deHTML((game.lastAction?.data.dice_result as GameActionDiceResult[])[0].description as string)).toBe(
        "targeting roll: target 4, rolled 36 [d10x10: 6 x 6]: hit"
      )
      expect(deHTML((game.lastAction?.data.dice_result as GameActionDiceResult[])[2].description as string)).toBe(
        "penetration roll (front): target 12, rolled 12 [2d10: 6 + 6]: tie, vehicle immobilized"
      )
      expect(game.fireStartCheckNeeded).toStrictEqual([])
    ...
    test("ranged fire against armored vehicle in contested hex", () => {
      const game = createFireGame()
      const map = game.scenario.map
      const firing = new Unit(testGTank)
      firing.id = "firing1"
      ...
      const tloc = new Coordinate(4, 4)
      map.addCounter(tloc, target)
      const other = new Unit(testGInf)
      other.id = "other1"
      map.addCounter(tloc, other)
      organizeStacks(map)
      ...
      const all = map.allUnits
      expect(all.length).toBe(3)
      expect(all[0].unit.id).toBe("firing1") // Wreck
      expect(all[1].unit.id).toBe("target1")
      expect(all[2].unit.id).toBe("other1")
  ...
  describe("cavalry", () => {
    test("carried infantry included when bicycle targeted", () => {
      ...
      const target = new Unit(testRBike)
      target.id = "target1"
      const tloc = new Coordinate(2, 2)
      map.addCounter(tloc, target)
      const target2 = new Unit(testRInf)
      ...
      expect(game.actions[0].stringValue).toBe(
        "German Rifle at E3 fired at Soviet Bicycle, Rifle at C3; target 12, " +
          "rolled 20 [2d10: 10 + 10]: critical hit, Bicycle destroyed"
      )
      expect(game.moraleChecksNeeded).toStrictEqual([{
    ...
    test("carried infantry included when horse targeted", () => {
      ...
      expect(game.actions[0].stringValue).toBe(
        "German Rifle at E3 fired at Soviet Horse, Rifle at C3; target 12, " +
          "rolled 20 [2d10: 10 + 10]: critical hit"
      )
      expect(game.moraleChecksNeeded).toStrictEqual([
