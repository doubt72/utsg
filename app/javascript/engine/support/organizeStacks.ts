...
function sortValues(game: Game, unit: Unit | Feature): number {
  ...
  if (unit.isFeature) { return -5 }
  const side = unit.playerNation === game?.playerOneNation ? 0 : 100
  const u = unit as Unit
  if (u.isWreck) { return side === 0 ? -3 : -2 }
  if (u.operated) { return side === 0 ? -1 : 0 }
  return ({
...
function addMarkers(
  ...
  if (!uf.isFeature) {
    ...
    if (unit.isTurretJammed) { markerTypes.push(markerType.TurretJammed) }
...
function pairCrewedWeapons(list: (Unit | Feature)[]): (Unit | Feature)[] {
  ...
  for (let index = 0; index < list.length; index++) {
    ...
    if (next && !next.isFeature &&
        ((unit.canCarrySupport && (!unit.decoy || next.decoy) && next.uncrewedSW && !(unit.leader &&
