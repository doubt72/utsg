...
export default function MainPhaseSection({ section }: SectionProps) {
    ...
    setShowInitiative(
      <div className="help-section-image">
        <svg width={203} height={653} viewBox="0 0 254 816" style={{ minWidth: 203 }}>
          <MapHexPatterns />
          <path
            d={roundedRectangle(1, 1, 252, 814, 8)}
  ...
  return (
    <div>
      <p>
        During the main phase of each turn, the player with initiative can take various actions (see
        the list below). Each action moves the marker on the initiative track, and after every
        action the acting player must make an initiative check, and if it fails, the initiative
        passes to the other player who may make their own actions in turn. Otherwise, the player can
        continue to order their units until no other orders are possible (or they otherwise choose
        to pass). Once both players pass back-to-back, the turn is over.
      </p>
      {showInitiative}
      <h3>{section}.1. Actions</h3>
      ...
      <ol>
        <li>
          ...
          towards the passing player, unless initiative is already on that player&apos;s side, in
          which case it passes control to the opposing player without moving the initiative marker.
        </li>
        <li>
      ...
      <p>
        ...
        track. Passing moves the initiative one space in the other direction towards the
        player&apos;s side (unless already on that side) and flips the initiative marker to the
        other player.
      </p>
      <h3>{section}.3. Snipers</h3>
      <p>
        If either player has a sniper, and the other player moves or fires an infantry unit, there
        will be a sniper check after the action but before the initiative check (except: assualt and
        rout actions don&apos;t trigger snipers). The player rolls two ten-sided dice, adding the
        total together (2d10), and if the number is equal or below the sniper roll on the marker,
        the player must make a morale check for any of their infantry units involved in the action
        (see Morale Checks in the {helpLink("Effects", "effects section")} of the docs).
      </p>
      <h3>{section}.4. Squads and Teams</h3>
      <p>
      ...
      <p>
        ...
        squad rounded down, and they don&apos;t get any of the special combat bonuses or abilities
        of the full squad.
      </p>
    </div>
  );
