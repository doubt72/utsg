...
export default function WeatherDisplay(props) {
    ...
    if (props.preview) {
      ...
      setBase(
        <g>
          ...
          <path d={baseCounterPath(props.x+10, props.y+10)}
                style={{ fill: "white", stroke: "black", strokeWidth: 1.5 }} />
          <text x={props.x + 15} y={props.y + 25} fontSize={16} textAnchor="left"
                fontFamily="'Courier Prime', monospace" style={{ fill: "black" }}>
            current
          </text>
          <path d={baseCounterPath(props.x+100, props.y+10)}
                style={{ fill: "white", stroke: "black", strokeWidth: 1.5 }} />
          <text x={props.x + 105} y={props.y + 25} fontSize={16} textAnchor="left"
                fontFamily="'Courier Prime', monospace" style={{ fill: "black" }}>
            precip
          </text>
          <polygon points={baseHexCoords(props.map || { radius: 0 }, props.x+95, props.y+170)}
                   style={{ fill: "white", stroke: "black", strokeWidth: 1.5 }} />
          {[1, 2, 3, 4, 5, 6].map(d => {
            if (!props.map) { return }
            const x = props.x + 95 - (props.map.radius-4) * Math.cos((d-1)/3 * Math.PI)
            const y = props.y + 170 - (props.map.radius-4) * Math.sin((d-1)/3 * Math.PI)
            return (
              <text key={d} fontSize={24} textAnchor="middle"
                    fontFamily="'Courier Prime', monospace" style={{ fill: "black" }}
                    transform={`translate(${x},${y}) rotate(${d*60 - 150})`}>
                {d}
              </text>
            )
          })}
          <text x={props.x + 95} y={props.y + 128} fontSize={16} textAnchor="middle"
                  fontFamily="'Courier Prime', monospace" style={{ fill: "black" }}>
            wind
          </text>
          <text x={props.x + 95} y={props.y + 144} fontSize={16} textAnchor="middle"
                  fontFamily="'Courier Prime', monospace" style={{ fill: "black" }}>
            direction
          </text>
        </g>
  ...
  }, [props.map.baseTerrain, props.map.night])
